...
import EclipseRocky from '@/models/eclipse_rocky'
...
export const POST = async (req: Request) => {
  ...
  try {
    ...
    const existingGame = await EclipseGame.findOne({ signature: body.signature, _id: { $ne: id } });
    if (existingGame) {
    ...
    const rocky = await EclipseRocky.findById(id)
    if (!rocky) {
    ...
    const game = await EclipseGame.create({
    ...
    if (game.outcome === "win") {
      ...
      description = `Congratulations! You chose ${game.choice} and the opponent chose ${cpuChoice}. You won ${winAmount} ETH! Claim your prize by clicking the button below now.`
      label = "Claim Prize!"
      ...
      winAmount = 0
      console.log("Win amount:", winAmount)
      description = `Unlucky! You chose ${game.choice} and the opponent chose ${cpuChoice}. You lost ${game.amount} ETH. Try your luck again!`
      label = "Play Again!"
    ...
    const payload: Action = (winAmount!=0) ? {
        ...
        actions: [
          {
            type: "transaction",
            label: label,
            href: `/api/actions/eclipse/won?id=${game._id.toString()}`
          }
        ]
      }
    } : {
      ...
      icon: image,
      description: `\nUnlucky! You chose ${game.choice} and the opponent chose ${cpuChoice}. You lost ${game.amount} ETH. Wanna try again?`,
      label: "",
      links: {
        actions: [
          {
